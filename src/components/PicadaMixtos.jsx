...
const PicadaMixtos = ({ onReturn, onNavigate }) => {
  ...
  const items = [
    {
      ...
      items: [
        { 
          ...
          price: "$45.000", 
          desc: "Variedad de carnes, chorizos, chicharrón y arepas",
          img: "/img/picada-casa.jpg"
        },
        { 
          name: "Mixto Especial", 
          price: "$38.000", 
          desc: "Combinación de carnes con patacones y guacamole",
          img: "/img/mixto-especial.jpg"
        },
      ]
    }
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row relative bg-gray-50">
      ...
      {/* Mitad azul claro */}
      <div className="w-full md:w-1/2 bg-gradient-to-br from-blue-400 to-blue-300 p-4 md:p-8 flex flex-col justify-center relative">
        <button 
        ...
        <motion.div 
          ...
          transition={{ delay: 0.3, duration: 0.6 }}
        >
          <div className="bg-blue-400 p-4 md:p-6 text-center">
          ...
          <div className="flex justify-center space-x-2 pb-4">
            {currentCategory.items.map((_, index) => (
              <div 
                key={index}
                className={`w-3 h-3 rounded-full ${index === currentItemIndex ? 'bg-blue-400' : 'bg-blue-200'}`}
              />
            ))}
          </div>
        </motion.div>
