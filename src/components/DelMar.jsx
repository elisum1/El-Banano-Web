...
const DelMar = ({ onReturn, onNavigate }) => {
  ...
  const delMarItems = [
    {
      ...
      items: [
        { 
          ...
          price: "$42.000", 
          desc: "Mezcla de mariscos en salsa de coco y aliños",
          img: "/img/cazuela-mariscos.jpg"
        },
        { 
          name: "Filete de Pescado", 
          price: "$38.000", 
          desc: "Filete a la plancha con salsa de alcaparras",
          img: "/img/filete-pescado.jpg"
        },
      ]
    }
  ];
  ...
  const navigateItems = (direction) => {
    const items = delMarItems[0].items;
    let newIndex;
  ...
  const currentCategory = delMarItems[0];
        ...
        >
          ...
          <div className="flex justify-center space-x-2 pb-4">
            {currentCategory.items.map((_, index) => (
              <div 
                key={index}
                className={`w-3 h-3 rounded-full ${index === currentItemIndex ? 'bg-blue-900' : 'bg-blue-200'}`}
              />
            ))}
          </div>
        </motion.div>
